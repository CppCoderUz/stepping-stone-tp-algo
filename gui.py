...
class TransportationSolveGUI:
    ...
    def __init__(self, solution_class, demand_name: str, offer_name: str, *args, **kwargs) -> None:
        super(TransportationSolveGUI, self).__init__()
        ...
        self.window = tk.Tk()
        self.window.title("Stepping stone usulida trasnport muammosini yechish usuli . ")
    ...
    def buttons_draw(self):
        ...
        self.menu.set(METHODS[0])
        
        self.dropdown = tk.OptionMenu(self.window, self.menu, *METHODS)
    ...
    def solve(self):
        ...
        try:
            point_list_ssm, point_list_lcm = self.solution_class(demand=talab, offer=taklif, costs=price_matrix).solve()
            
            if self.menu.get() == METHODS[0]:
                point_list_ssm = list(map(price_real_or_int, point_list_ssm))
                self.solve_view(point_list_ssm, price_matrix)
            else:
                point_list_lcm = list(map(price_real_or_int, point_list_lcm))
                self.solve_view(point_list_lcm, price_matrix)
